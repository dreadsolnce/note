...
3. Установить (проверено на rhel 7.9):
```
sudo yum localinstall https://repo.zabbix.com/zabbix/6.2/rhel/9/x86_64/zabbix-agent2-6.2.9-release1.el9.x86_64.rpm
```
...
- `ServerActive` — продублировать доменное имя или IP-адрес, на котором находится сервер Zabbix. Данный параметр отвечает за активный режим, когда Zabbix самостоятельно запрашивает необходимые данные. `ServerActive` - для активных проверок, когда агент сам отправляет данные на сервер (***не применяем можно закоментировать***)
