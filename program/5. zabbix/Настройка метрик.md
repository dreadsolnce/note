...
***1 ВАРИАНТ: в общем конфигурационном файле отработанный***
...
```
curl http://127.0.0.1:8900/basic_status
```
...
Прописать MACROS для хоста (port и host)
![[Screenshot_171.png]]
...
```bash
sudo zabbix_agent2 -t 'fail2ban.status[sshd]' && sudo zabbix_agent2 -t 'fail2ban.discovery'
```
...
**Проверка текущих значений параметров**

```bash
sudo fail2ban-client get sshd bantime && sudo fail2ban-client get sshd findtime && sudo fail2ban-client get sshd maxretry
```
...
**ЭТО ГОТОВЫЙ ШАБЛОН ГДЕ УЖЕ ПРОПИСАНЫ fail2ban.discovery fail2ban.total_banned fail2ban.currently_failed**

**А МОЖНО ДОБАВИТЬ ПАРАМЕТРЫ В УЖЕ ИМЕЮЩИЕСЯ ШАБЛОН**

![[Снимок экрана от 2026-03-26 12-49-07.png]]

![[Снимок экрана от 2026-03-26 12-49-25.png]]

![[Снимок экрана от 2026-03-26 12-49-38.png]]

![[Снимок экрана от 2026-03-26 12-49-49.png]]

![[Снимок экрана от 2026-03-26 12-50-03.png]]

![[Снимок экрана от 2026-03-26 12-50-14.png]]
...
### 1. Если запущен через unix сокет:
...
![[Снимок экрана от 2025-10-17 18-06-29.png]]
...
На zabbix сервере сделать если нет clone template PHP-FPM by Zabbix agent под названием PHP-FPM by Zabbix agent UNix

И в новом темплате заменить значение Macros {$PHP_FPM.HOST} с localhost на 127.0.0.1
![[Screenshot_164.png]]
![[Screenshot_165.png]]
