...
#### Включение папки в игнорирование:

Если папка уже была добавлена в Git ранее, простого добавления в `.gitignore` недостаточно. Необходимо убрать её из индекса, но оставить локально:

```
# Удалить папку из индекса (но не с диска)
git rm -r --cached папка/

# Закоммитить изменения
git commit -m "Удалена папка из отслеживания"
```


> [!NOTE] Title
> - `folder_name/` — игнорировать всю папку `folder_name` и её содержимое в любом месте проекта.
> - `/folder_name/` — игнорировать папку `folder_name` только в корне проекта
