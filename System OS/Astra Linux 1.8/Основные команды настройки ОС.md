...
```
sudo nmcli connection modify Wired\ connection\ 1 ipv4.dns "172.16.65.69"
```
...
Просмотр созданных dns записей:
```
sudo nmcli connection show Wired\ connection\ 1 | grep dns
```

Проверка какие dns используются:
```
nmcli dev show | grep DNS
```

Для вступления настроек в силу необходимо перезапустить интерфейс:
