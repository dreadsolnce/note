...
![[Снимок экрана от 2025-08-14 18-19-50.png]]
...
```
docker run -d --rm -p 127.0.0.1:5001:5000 --name app app:1.0.0
```
...
services:  
  web:  
    image: cr.yandex/crp12342uggr50sj0bqu/app:1.0.0  
    container_name: app  
    hostname: app  
    environment:  
      - DB_HOST=db  
...
```
docker run --rm --entrypoint "" -v /opt/backup:/backup --link=db:alias --network=app_backend my-mysqldump mysqldump --opt -h alias -u root -p"YtReWq4321" "--result-file=/backup/dumps.sql" virtd
```
...
```
docker run -it hashicorp/terraform:latest terraform version
```
