...
| sudo useradd -m -s /bin/bash -G sudo,astra-admin <имя_пользователя> && echo "<имя_пользователя>:<пароль>" \| sudo chpasswd |                                                                            |                                                             |
...
*Поиск по названию файлов содержащих в названии часть слова*

```
sudo find . -name "*<строка для поиска>*"
```
...
*Поиск (вывод) файла (test.txt) если он был модифицирован 3 минуты назад*

```
find "test.txt" -mmin +3
```
...
*Количество работающих worker-ов конкретной службы:*

```
systemctl show gunicorn.service --property=TasksCurrent --value
```
