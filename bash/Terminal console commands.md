...
| useradd -m -s /bin/bash -p `<br>-p $(openssl passwd -1 password)<br> -G wheel kolchinvll |                                                                            |                                                             |
...
*Список процессов по потребляемой памяти:*
```
ps -AH -o pid,size,command | sort -rn -k2 | grep php-fpm
```
> во втором столбце указана потребляемая память в килобайтах

Количество общей и используемой памяти:
```
free -lh
```
...
- `sudo shutdown -r +5` — перезагрузит систему через 5 минут.
- `sudo shutdown -r 12:00` — перезагрузит систему в 12:00.

**Как отменить перезагрузку:**

- `sudo shutdown -c` — отменит запланированную перезагрузку
##### ***==Сеть==***
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;*Вывод ай пи адреса хоста:*
```
hostname -I    
```
...
Снизить mtu серевого интерфейса:
...
Отправка запроса к локальному серверу с указанием имени домена
```
curl -v -k -H "Host: www.dev.kvlpro.ru" https://127.0.0.1/example
```
...
Изменение действия интервала пароля sudo (файл sudoers):
```
...
Вывод файлов с сортировкой по дате по возрастанию (-r меняет порядок сортировки)

```
ls -altr /usr/local/share/backup/www.conf.*
```
...
Параметр ***RuntimeDirectory*** в службе:
```
systemctl edit renderd.service
```

> [!NOTE] Описание
> `systemctl RuntimeDirectory`
> — это ==**настроенное значение в файле юнита systemd (например, `*.service`), которое указывает на каталог, создаваемый и монтируемый в оперативной памяти (`tmpfs`) на время выполнения службы**==. Такой подход позволяет службе хранить временные данные в памяти, что обеспечивает более быстрый доступ и автоматически очищает каталог при перезагрузке. **Создание каталога:** При запуске службы `systemd` автоматически создает каталог `/run/<имя_каталога>`.

![[Снимок экрана от 2025-11-13 15-56-08.png]]
...
***Назначить Sticky Bit на папку (аналогично папке /tmp), который позволяет в нее писать всем, а вот удалять только либо создателю ресурса либо пользователю root:***
...
***Предоставьте разрешения пользователю postgres:***
